...
func (o *UserRole) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	...
	values := []interface{}{related.UserID, o.UserID, o.RoleName}
...
func UserRoles(mods ...qm.QueryMod) userRoleQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`user_role`.*"})
	}
...
func FindUserRole(ctx context.Context, exec boil.ContextExecutor, userID int, roleName string, selectCols ...string) (*UserRole, error) {
	...
	query := fmt.Sprintf(
		"select %s from `user_role` where `user_id`=? AND `role_name`=?", sel,
	)

	q := queries.Raw(query, userID, roleName)
...
func (o *UserRole) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `user_role` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `user_role` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, userRolePrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for user_role")
	}
...
func (o *UserRole) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`user_role`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `user_role` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for user_role")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for user_role")
	}
...
func (o *UserRole) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `user_role` WHERE `user_id`=? AND `role_name`=?"
...
func (o *UserRole) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindUserRole(ctx, exec, o.UserID, o.RoleName)
	if err != nil {
		return err
	}
...
func UserRoleExists(ctx context.Context, exec boil.ContextExecutor, userID int, roleName string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `user_role` where `user_id`=? AND `role_name`=? limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, userID, roleName)
	}
	row := exec.QueryRowContext(ctx, sql, userID, roleName)
...
func (o *UserRole) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return UserRoleExists(ctx, exec, o.UserID, o.RoleName)
}
