...
func testUserToManyCreatedByTickets(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	b.CreatedBy = a.UserID
	c.CreatedBy = a.UserID
	...
	check, err := a.CreatedByTickets().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadCreatedByTickets(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CreatedByTickets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.CreatedByTickets = nil
	if err = a.L.LoadCreatedByTickets(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CreatedByTickets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyUsedByTickets(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.UsedBy, a.UserID)
	queries.Assign(&c.UsedBy, a.UserID)
	if err = b.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
	if err = c.Insert(ctx, tx, boil.Infer()); err != nil {
	...
	check, err := a.UsedByTickets().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if queries.Equal(v.UsedBy, b.UsedBy) {
			bFound = true
		}
		if queries.Equal(v.UsedBy, c.UsedBy) {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUsedByTickets(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UsedByTickets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UsedByTickets = nil
	if err = a.L.LoadUsedByTickets(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UsedByTickets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyUserAgreements(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserAgreements(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyUserAuthorities(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserAuthorities(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpCreatedByTickets(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddCreatedByTickets(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.UserID != first.CreatedBy {
			t.Error("foreign key was wrong value", a.UserID, first.CreatedBy)
		}
		if a.UserID != second.CreatedBy {
			t.Error("foreign key was wrong value", a.UserID, second.CreatedBy)
		}
		...
		if a.R.CreatedByTickets[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.CreatedByTickets[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.CreatedByTickets().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpUsedByTickets(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddUsedByTickets(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.UserID, first.UsedBy) {
			t.Error("foreign key was wrong value", a.UserID, first.UsedBy)
		}
		if !queries.Equal(a.UserID, second.UsedBy) {
			t.Error("foreign key was wrong value", a.UserID, second.UsedBy)
		}

		if first.R.UsedByUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.UsedByUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.UsedByTickets[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UsedByTickets[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UsedByTickets().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManySetOpUsedByTickets(t *testing.T) {
	var err error
	...
	err = a.SetUsedByTickets(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetUsedByTickets(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.UsedBy) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.UsedBy) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.UserID, d.UsedBy) {
		t.Error("foreign key was wrong value", a.UserID, d.UsedBy)
	}
	if !queries.Equal(a.UserID, e.UsedBy) {
		t.Error("foreign key was wrong value", a.UserID, e.UsedBy)
	}

	if b.R.UsedByUser != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.UsedByUser != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.UsedByUser != &a {
	...
	if a.R.UsedByTickets[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.UsedByTickets[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpUsedByTickets(t *testing.T) {
	var err error
	...
	err = a.AddUsedByTickets(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveUsedByTickets(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.UsedByTickets) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.UsedByTickets[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.UsedByTickets[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testUserToManyAddOpUserAgreements(t *testing.T) {
	var err error
...
func testUserToManyAddOpUserAuthorities(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`UserID`: `int`, `Name`: `varchar`, `AuthorizedBy`: `enum('GOOGLE')`, `AuthorizedID`: `varchar`, `Email`: `varchar`, `CreateDate`: `datetime`}
	_           = bytes.MinRead
)
