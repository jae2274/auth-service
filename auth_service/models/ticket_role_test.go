...
func testTicketRolesExists(t *testing.T) {
	...
	e, err := TicketRoleExists(ctx, tx, o.TicketID, o.RoleName)
	if err != nil {
...
func testTicketRolesFind(t *testing.T) {
	...
	ticketRoleFound, err := FindTicketRole(ctx, tx, o.TicketID, o.RoleName)
	if err != nil {
		t.Error(err)
	}
...
func testTicketRoleToOneSetOpTicketUsingTicket(t *testing.T) {
	...
	for i, x := range []*Ticket{&b, &c} {
		...
		if exists, err := TicketRoleExists(ctx, tx, a.TicketID, a.RoleName); err != nil {
			t.Fatal(err)
		} else if !exists {
