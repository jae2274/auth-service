...
func testUserRolesExists(t *testing.T) {
	...
	e, err := UserRoleExists(ctx, tx, o.UserID, o.RoleName)
	if err != nil {
...
func testUserRolesFind(t *testing.T) {
	...
	userRoleFound, err := FindUserRole(ctx, tx, o.UserID, o.RoleName)
	if err != nil {
		t.Error(err)
	}
...
func testUserRoleToOneSetOpUserUsingUser(t *testing.T) {
	...
	for i, x := range []*User{&b, &c} {
		...
		if exists, err := UserRoleExists(ctx, tx, a.UserID, a.RoleName); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	userRoleDBTypes = map[string]string{`UserID`: `int`, `RoleName`: `varchar`, `GrantedType`: `enum('ADMIN','TICKET')`, `GrantedBy`: `int`, `ExpiryDate`: `datetime`}
	_               = bytes.MinRead
)
